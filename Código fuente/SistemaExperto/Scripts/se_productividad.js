...
var estaciones_sty = (function (feature) {
    var style_1 = new ol.style.Style({
        image: new ol.style.Icon({
            anchor: [0.5, 1],
            src: raiz + '/Content/imagenes/Iconos/estacionIcono.png'
        })
    });
    return [style_1];
})
...
function agregarEstacion(codigoIdeam) {
    quitarEstaciones();
    arregloCapas.push({
        "nombre": "estaciones_climatologicas",
        "url": "estaciones_climatologicas",
        "filtro": "codigo='" + codigoIdeam.replace('ó', 'o').toUpperCase() + "'",
...
function quitarEstaciones() {
    if (arregloCapas.length > 1) {
        map.removeLayer(arregloCapas[1].capa);
        arregloCapas.splice(1, 1);
    }
}
...
//    var fuente = arregloCapas[codigo].fuente;
